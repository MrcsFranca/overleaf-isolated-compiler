/**
 * Overleaf CE — Custom Settings for Sandbox Compile
 * ====================================================
 * This file substitute the default /etc/overleaf/settings.js inside the sharelatex container.
 * It's mounted via volume on the docker-compose.base.yml
 *
 * The only change from the Overleaf CE settings.js is in apis.clsi.url:
 * Instead of using intern CLSI from container (http//127.0.0.1:3013),
 * it's redirected for the extern proxy that creates isolated sandox containers
 *
 * To update this file with a new version of Overleaf CE, copy the default settings.js
 * from the new image and change only the line marked as [SANDBOX]
 *
 * How to obtain the standart settings.js from the installed version:
 *   docker cp sharelatex:/etc/overleaf/settings.js ./proxy/settings.custom.js
 */

// ─── Begin of default Overleaf CE ─────────────────────────────
...
const settings = {
    ...
    apis: {
        // [SANDBOX] ← THE ONLY CHANGE MADE
        // Redirects all the compiles to the to the proxy which creates the sandbox containers
        // Default would be: url: 'http://127.0.0.1:3013'
        clsi: {
            url: 'http://proxy:3013',
        },
        web: {
